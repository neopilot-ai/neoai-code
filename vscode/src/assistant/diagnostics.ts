...
async function refreshDiagnostics(
  ...
  try {
    ...
    const newDiagnostics: NeoAiDiagnostic[] = [];
    assistantDiagnostics?.forEach((assistantDiagnostic) => {
      ...
      if (choices.length > 0) {
        ...
        if (
          ...
          const diagnostic = new NeoAiDiagnostic(
            vscodeRange,
            `${choicesString.join("\n")}`,
            choices,
          ...
          diagnostic.code = NEOAI_DIAGNOSTIC_CODE;
    ...
    setStatusBarMessage();
    Logger.error(`neoai assistant: error: ${(e as Error).toString()}`);
  } finally {
    lock();
  }
}
...
function refreshDiagnosticsOrPrefetch(
  document: vscode.TextDocument,
  neoAiDiagnostics: vscode.DiagnosticCollection
) {
...
export default async function registerAssistant(
  ...
  const neoaiDiagnostics = vscode.languages.createDiagnosticCollection(
    "neoai"
  );
  context.subscriptions.push(neoaiDiagnostics);
  const documentValidator = await getValidator();

  initAssistantThreshold(context);

  registerRefreshCommand(context, documentValidator, neoaiDiagnostics);

  registerAssistantModeToggle(neoaiDiagnostics, documentValidator);

  // handleActiveEditorChanged(context, documentValidator, neoaiDiagnostics);

  handleVisibleRangeChange(context, documentValidator, neoaiDiagnostics);

  pasteDisposable.dispose();
  registerPasteCommand(context, documentValidator, neoaiDiagnostics);

  // For AssistantMode.Paste
  handlePasteChange(context, neoaiDiagnostics, documentValidator);

  // For AssistantMode.Background
  handleTextChange(context, neoaiDiagnostics, documentValidator);
      ...
      if (editor) {
        ...
        if (documentValidator.isValid(document)) {
          if (getAssistantMode() === AssistantMode.Paste) {
            if (currentRange) {
              void refreshDiagnostics(document, neoaiDiagnostics, [
                currentRange?.range,
              ]);
            }
          } else {
...
function registerAssistantModeToggle(
  ...
  vscode.commands.registerCommand(ASSISTANT_MODE_TOGGLE_COMMAND, () => {
    cancellationToken.cancel();
    if (vscode.window.activeTextEditor) {
      neoAiDiagnostics.delete(vscode.window.activeTextEditor.document.uri);
    }
    setDecorators([]);
    const newMode =
      getAssistantMode() === AssistantMode.Background
    ...
    if (getAssistantMode() === AssistantMode.Paste) {
      void vscode.window.showInformationMessage("neoai assistant paste mode");
      Logger.info("paste validation mode");
    } else {
      void vscode.window.showInformationMessage(
        "neoai assistant background mode"
      );
    ...
    vscode.commands.registerCommand(
      ...
      async (textEditor: vscode.TextEditor) => {
        ...
        if (vscode.window.activeTextEditor) {
          ...
          inPaste = false;
          neoAiDiagnostics.delete(document.uri);
          setDecorators([]);
          void refreshDiagnostics(document, neoAiDiagnostics, [
            currentRange.range,
          ]);
