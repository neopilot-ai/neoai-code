...
let BINARY_UPDATE_URL = "https://update.neoai.com/bundles";
...
function getSuffix(): string {
  switch (process.platform) {
    case "win32":
      return "pc-windows-gnu/NeoAi.exe";
    case "darwin":
      return "apple-darwin/NeoAi";
    case "linux":
      return "unknown-linux-musl/NeoAi";
    default:
      throw new Error(
        `Sorry, the platform '${process.platform}' is not supported by NeoAi.`
      );
  }
}
...
function getArch(): string {
  ...
  throw new Error(
    `Sorry, the architecture '${process.arch}' is not supported by NeoAi.`
  );
}
