...
import NeoaiSession from "./NeoaiSession";
...
function toSession({ accessToken, username }: UserAuthData): NeoaiSession {
  return new NeoaiSession(username, accessToken);
}

const setAuthenticationReadyOnce = runOnce(setAuthenticationReady);

export default class NeoaiAuthenticationProvider
